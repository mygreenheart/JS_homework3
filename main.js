let arrList = [{ ratingRevievs: "264 отзыва", price: { oldUan: "4 333 грн", newUan: "3 799 грн" }, name: "Motorola MOTO G4 (XT1622) Black" },
...
//Преобразование отзывов в числовой формат

//Алгоритм сортировки
function SortByRating() {
    arrList.map(function (e) {
        e.ratingRevievs = Number(e.ratingRevievs.match(/(\d+)/g));
    })
    arrList.sort((a, b) => a.ratingRevievs > b.ratingRevievs ? 1 : -1);
    document.write('<div className="tracks-table"><table className="table">');
    for (var i = 0; i < arrList.length; i++) {
        var name = arrList[i].name;

        if (arrList[i].price.newUan != undefined) {
            var price = arrList[i].price.newUan;
        } else {
            price = arrList[i].price
        }

        var ratingRevievs = arrList[i].ratingRevievs;

        document.write('<tr className="mess-hide"><td className="name"><h4>' + name + '</h4></td><td className="song"><h4>' + price + '</h4></td><td className="url"><h4>' + ratingRevievs + ' отзывов </h4></td></tr>');
    }
}
...
function SortByPrice() {
    arrList.map(function (e) {
        if (e.price.newUan) {
            e.price = e.price.newUan;
        }
        //Убирание пробелов
        e.price = e.price.replace(/\s*/g, '');
        e.price = Number(e.price.match(/(\d+)/g));
    })
    arrList.sort((a, b) => a.price > b.price ? 1 : -1);
    document.write('<div className="tracks-table"><table className="table">');
    for (var i = 0; i < arrList.length; i++) {
        var name = arrList[i].name;
        if (arrList[i].price.newUan != undefined) {
        ...
        var ratingRevievs = arrList[i].ratingRevievs;
        document.write('<tr className="mess-hide"><td className="name"><h4>' + name + '</h4></td><td className="song"><h4>' + price + ' грн </h4></td><td className="url"><h4>' + ratingRevievs + '</h4></td></tr>');
    }
}
